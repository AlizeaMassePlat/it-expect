...
const userController = {
  ...
  async register(req, res) {
    try {
      ...
      // Check if newUser is null or if rowCount is 0
      if (!newUser || newUser.rowCount === 0) {
  ...
  async resetPassword(req, res) {
    try {
      ...
      // Await the sendMail function if it returns a Promise
      await new Promise((resolve, reject) => {
        contactEmail.sendMail(mailOptions(email, newTokens.accessToken), (error) => {
          if (error) {
            res.json({
              message:
